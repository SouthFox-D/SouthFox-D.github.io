...
(define (navbar)
  `(nav (@ (class "nav") (aria-label ,(t_ 'aria-navbar)))
    (a (@ (class "skip-link") (href "#main-content")) ,(t_ 'aria-skip))
    (a (@ (href "/")
          (class "brand")
          (aria-label ,(string-append "狐狸反走矣 - " (t_ 'aria-home))))
       (span "狐狸反走矣"))
    (input (@ (id "bmenub")
              (type "checkbox")
              (class "show")
              (aria-label ,(t_ 'aria-menu))))
    (label (@ (for "bmenub")
         ...
         (ul
          ...
          (li (a (@ (href "https://www.fsf.org/appeal"))
                 (img (@ (src "/assets/img/6838639.png")
                         ...
                         (height 45)
                         (alt "FSF appeal"))))))
         (nav (@ (aria-label "IndieWeb WebRing"))
              (a (@ (href "https://xn--sr8hvo.ws/previous")
                    (aria-label ,(format (t_ 'aria-previous-site) "IndieWeb")))
                 "←")
              (a (@ (href "https://xn--sr8hvo.ws"))
                 "IndieWeb Webring" (span (@ (aria-hidden "true")) "💍"))
              (a (@ (href "https://xn--sr8hvo.ws/next")
                    (aria-label ,(format (t_ 'aria-next-site) "IndieWeb")))
                 "→"))
         (nav (@ (aria-label "Fediring"))
              (a (@ (href "https://fediring.net/previous?host=blog.southfox.me")
                    (aria-label ,(format (t_ 'aria-previous-site) "FediRing")))
                 "←")
              (a (@ (href "https://fediring.net"))
                 "Fediring")
              (a (@ (href "https://fediring.net/next?host=blog.southfox.me")
                    (aria-label ,(format (t_ 'aria-next-site) "FediRing")))
                 "→")))
    (div (@ (class "widget"))
         (h4 ,(t_ 'settings))
         (div
          (label (@ (for theme-select)) ,(t_ 'theme-setting))
          (select (@ (id "theme-select"))
                  (option (@ (value "plain")) ,(t_ 'plain))
                  (option (@ (value "386")) "386")
                  (option (@ (value "frutiger-aero")) "Frutiger Aero")
                  ))
         (div
          (label (@ (for font-select)) ,(t_ 'font-setting))
          (select (@ (id "font-select"))
                  (option (@ (value "plain")) ,(t_ 'plain))
                  (option (@ (value "zpix")) ,(t_ 'pixel)))))
    ,(if post
         `(div (@ (class "widget"))
           (h4 ,(t_ 'tags))
           (ul
           ...
           (script (@ (type "text/x-scheme") (defer "defer"))
                   (lips (let ((theme (or (localStorage.getItem "theme") "386"))
                           ...
                           (let ((theme-select (document.getElementById "theme-select"))
                             ...
                             (loop-set theme-select theme)
                             (loop-set font-select font)
          ...
          (div
           ...
           (p (@ (class "post-meta"))
              ...
              (time (@ (data-pagefind-sort "date"))
                    ,(date->string (post-date post) (t_ 'date))))
           (div ,(post-sxml post))))
