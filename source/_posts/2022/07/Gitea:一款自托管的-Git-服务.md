...
确认无误后使用 `docker-compose up -d` 启动全部服务，之后前往 `https://cicd.git.server` 地址查看是否正常运行，是的话点击登陆，看是否能够和 `Gitea` 进行联动，如果显示错误请检查是否与上述配置一致（例如某个路径多带了一个 `/` 导致之后生成的验证路径出错，去掉 `/` 之后就正常了）。
...
如果像我什么都没改的话，想要克隆仓库就会得到这么一可奇怪的地址：
