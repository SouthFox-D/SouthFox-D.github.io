...
```
 ...
 其中 `tag` 应该去 [`Docker Hub` 页面](https://hub.docker.com/_/elasticsearch)寻找，因为没有默认标签的样子只能手动指定版本了……而且注意要选修了高危漏洞的版本。
...
并且 `systemctl restart td-agent` 重启应用，开放相应处理让 `Docker`  能够进行联协，并将相应数据通过 `9200` 端口汇入到 `Elasticsearch` 进行处理。
...
不过难受的点就是，`EFK` 名头 那么大，但是相关文档真的很少（就算用英文搜索）……感觉自己在用一个过气项目的样子，不知道是不是我不得要领……
