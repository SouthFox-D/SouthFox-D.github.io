...
在不断翻列表和按星星数、按语言排序后，最后看上了 [[https://dthompson.us/projects/haunt.html][Haunt]] ，一个用 Guile Scheme 写的静态网站生成器。在体验了几下后我决定，
...
有这种语法，就不需要什么 EJS 或者 GO 模板文件还有什么 =${...}= 了，模板和代码可以轻松交织在一起：
...
不过还好里面的流程是可以替换的，所以我往文本解析器的加了 =shortcode= （叫法来源 Hugo) 的规则，匹配类似
