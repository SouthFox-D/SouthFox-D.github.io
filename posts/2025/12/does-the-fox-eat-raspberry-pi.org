...
也可以是自己编写的复杂的例如生成一大坨 docker-compose.yml 文件的函数。而且通过 python 作为通用编程语言的优势很容易将这些 service 组合成
一个顶层的函数以声明式配置方式在最后区执行，并且还能通过一些获取例如主机名进行分支控制达到一套代码库在在不同机器部署不同环境的结果，例如：
